...
export const SystemSetup: React.FC<SystemSetupProps> = ({ onSetupCompleted, onCancel }) => {
  ...
  // Form State for Wizard Creation
  ...
  // Helper to seal system setup in Firestore
  const sealSystemSetup = async (superAdminEmails: string[]) => {
    ...
    try {
      await addDoc(collection(db, 'auditLogs'), {
        userId: auth.currentUser?.uid || 'sys',
        actorName: auth.currentUser?.displayName || 'System Core',
        userRole: 'Super Administrator',
        action: 'SYSTEM_INIT_PERMANENTLY_SEALED',
        details: `System initialization sealed with Super Administrators (${superAdminEmails.join(', ')}). Setup wizard disabled.`,
  ...
  // Handler for Existing Admin Sign-In Mode
  const handleExistingAdminSignIn = async (e: React.FormEvent) => {
    e.preventDefault();
    const cleanEmail = existingEmail.trim().toLowerCase();
    const cleanPass = existingPassword.trim();

    if (!cleanEmail || !cleanPass) {
      setErrorMsg('Please enter your existing Super Administrator email and password.');
      return;
    }
  ...
  // Handler for Wizard Account Creation Mode
      ...
      <div className="system-initialization-wizard w-full max-w-2xl bg-slate-900/90 border border-slate-800/80 rounded-3xl p-6 sm:p-9 shadow-2xl relative z-10 space-y-6 backdrop-blur-md">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs sm:text-sm text-slate-400 max-w-lg mx-auto leading-relaxed">
            Provision the required <strong className="text-white">Super Administrator accounts</strong> to establish governance. Once complete, this setup page will be <strong className="text-rose-400">permanently disabled</strong>.
          </p>
        </div>
        ...
        {superAdminCount >= 1 && setupMode === 'create' && (
          <div className="p-3.5 bg-amber-500/10 border border-amber-500/30 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-amber-200 animate-fadeIn">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-300/80">You can sign in directly using an existing Super Admin account.</p>
              </div>
            </div>
            <button
              type="button"
        ...
        {/* MODE: SIGN IN WITH EXISTING SUPER ADMINISTRATOR */}
        {setupMode === 'existing' && (
          <form onSubmit={handleExistingAdminSignIn} className="space-y-4 animate-fadeIn">
            <div className="p-3.5 rounded-2xl bg-slate-950 border border-amber-500/30 text-xs space-y-1">
              <p className="font-bold text-amber-400">Authenticate Existing Firebase Account</p>
              <p className="text-slate-400 text-[11px] leading-relaxed">
                If your account was previously created in Firebase Authentication, enter your credentials below to authenticate, promote your profile to Super Administrator in Firestore, and launch the portal.
              </p>
            </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="e.g. admin@kenfoss.co.ke"
                    className="w-full pl-10 pr-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-amber-500 transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••••••"
                    className="w-full pl-10 pr-10 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-amber-500 transition-colors font-mono"
                  />
                  <button
                    type="button"
            ...
            <button
              type="submit"
              disabled={isLoading || !existingEmail || !existingPassword}
              className={`w-full py-3 rounded-2xl font-bold text-xs transition-all shadow-lg flex items-center justify-center space-x-2 cursor-pointer ${
                existingEmail && existingPassword && !isLoading
            ...
            >
              {isLoading ? (
                <span>Authenticating Existing Super Admin...</span>
              ) : (
                <>
                  <LogIn className="w-4 h-4" />
                  <span>Authenticate & Seal System Setup</span>
                </>
              )}
            </button>
          </form>
        ...
        {/* MODE: CREATE NEW SUPER ADMINISTRATOR WIZARD */}
        {setupMode === 'create' && (
          <>
              ...
              <div className="space-y-6 text-center py-2 animate-fadeIn">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-300 max-w-md mx-auto leading-relaxed">
                    Super Administrator accounts have been registered in Firebase Authentication & Firestore.
                  </p>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-950 border border-rose-500/20 text-xs text-slate-300 space-y-2 text-left">
                  ...
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    The setup page is now permanently locked in Firestore Security Rules. Public role self-escalation is blocked. Staff accounts can only be created from inside the Staff Management module after logging in as a Super Administrator.
                  </p>
                </div>
